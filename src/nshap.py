...
from nShapleyValues import nShapleyValues
import vfunc as vfunc
from vfunc import memoized_vfunc
from plot import plot_n_shapley
from util import allclose, save, load, powerset
...
def n_shapley_values(X, v_func, n=-1):
    ...
    # return result
    results = [nShapleyValues(x) for x in results]
    if len(results) == 1:
        return results[0]
    return results
...
def shapley_gam(X, v_func):
    ...
    # return result
    result = [nShapleyValues(x) for x in result]
    if len(result) == 1:
        return result[0]
    return result
